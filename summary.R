...
king_county_data <- incarcceration_df %>% 
  select(county_name, year, total_pop_15to64, female_pop_15to64, male_pop_15to64, black_pop_15to64, 
      latinx_pop_15to64, native_pop_15to64, white_pop_15to64, aapi_pop_15to64, other_race_prison_pop, ) %>% 
...
WA_gender_in_priosn <- incarcceration_df %>% 
  select(state, year, county_name, female_pop_15to64, male_pop_15to64, total_pop, 
         female_prison_pop, male_prison_pop) %>% 
  filter(state == "WA")
...
WA_jail_to_prison <- incarcceration_df %>% 
  select(state, year, county_name, total_jail_pop, female_jail_pop, male_jail_pop,
         black_jail_pop, white_jail_pop, latinx_jail_pop, native_jail_pop, aapi_jail_pop) %>% 
  filter(state == "WA")
